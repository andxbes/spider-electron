...
> Останнє оновлення: 2026-06-14 (збереження title/meta/h1 при повторному upsert)  
...
└── renderer/
    ...
    ├── table-view.js      # Компонент таблиці (lazy render, хуки колонок)
    ├── detail-panel.js    # Панель деталей (хуки рядків)
    ├── export-csv.js      # Експорт через ui:exportColumns
    ├── renderer.js        # Оркестрація: IPC, фільтри, керування сканом
...
| `renderer.js` | DOM, IPC callbacks, фільтри, workspace, старт скану |
...
- `renderer.js` ще містить логіку таблиці поруч із `table-view.js` (можна повністю делегувати в наступному кроці).
- UI українською.
